...
describe('Test respostories page', () => {
   beforeEach(()=> { 
     window.localStorage.setItem('auth-code', "test")
     window.localStorage.setItem('access-token', "test")
     ...
     cy.server()
      ...
      cy.server()
      cy.route('https://cors-anywhere.herokuapp.com/https://api.github.com/user/repos?test2', 
      [
        {
           "id":204345349,
           "node_id":"MDEwOlJlcG9zaXRvcnkyMDQzNDUzNDk=",
   ...
   it('Check for add repo', () => {
      window.localStorage.setItem('access-token', "test2")
